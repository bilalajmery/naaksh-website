...
export default function Shop() {
  ...
  useEffect(() => {
    ...
    const validCategories = categories.filter((cat) =>
      incoming.some((inc) => inc.toLowerCase() === cat.name.toLowerCase())
    );
  ...
  const filteredProducts = useMemo(() => {
    return products.filter((product) => {
      ...
      if (filters.categories.length > 0) {
        const match = filters.categories.some((cat) => product.category.toLowerCase() === cat.toLowerCase());
  ...
  const sortedProducts = useMemo(() => {
    ...
    return list.sort((a, b) => (b.id || 0) - (a.id || 0)); // newest (highest id first)
        ...
        <div className="mb-12">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
            <p className="text-gray-600">
              Showing {currentProducts.length > 0 ? (currentPage - 1) * productsPerPage + 1 : 0}–
              {Math.min(currentPage * productsPerPage, sortedProducts.length)} of {sortedProducts.length}{" "}
              products
            </p>
            <select
              value={sortBy}
          ...
          <aside className="hidden lg:block w-72 shrink-0 sticky top-24">
            <div className="bg-white border border-gray-200 rounded-lg shadow-sm p-6">
                ...
                {categories.map((cat) => (
                  <label key={cat.name} className="flex items-center justify-between cursor-pointer">
                    <div className="flex items-center gap-3">
                      <input
                        type="checkbox"
                        checked={filters.categories.some((c) => c.toLowerCase() === cat?.name.toLowerCase())}
              ...
              <div className="mb-6 pb-6 border-b border-gray-200">
                <h3 className="text-xs font-semibold mb-4 uppercase tracking-widest text-gray-900">
                  Price Range
                </h3>
